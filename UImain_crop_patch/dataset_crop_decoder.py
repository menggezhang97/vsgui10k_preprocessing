...
def should_filter_ui_element(
    x1: float, y1: float, x2: float, y2: float,
    seg_w: float, seg_h: float,
    ui_type: str,
...
def encode_ui_elements(
    ...
    for elem in elements:
        ...
        if should_filter_ui_element(
            x1=x1, y1=y1, x2=x2, y2=y2,
            seg_w=seg_w, seg_h=seg_h,
            ui_type=ui_type,
            drop_full_screen_root=drop_full_screen_root,
        ):
            continue
        ...
        kept_elements.append({
            "bbox": [x1, y1, x2, y2],   # still in segmentation coordinate system
            "type": ui_type,
        })
...
def crop_ui_regions(
    ...
    """
    IMPORTANT:
    kept_elements bbox is in segmentation coordinate system.
    We must map bbox -> original image coordinate system before cropping.
    """
    crops = []
    ...
    for elem in kept_elements:
        ...
        # map segmentation coords -> original image coords
        ...
        crop = crop_transform(crop)  # [3, crop_size, crop_size]
        crops.append(crop)
    ...
    return torch.stack(crops, dim=0)  # [K, 3, crop_size, crop_size]
...
class HybridPlainDecoderDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        return {
            ...
            "ui_mask": ui_mask,
            "ui_crop_images": ui_crop_images,  # [K, 3, crop_size, crop_size]
        }