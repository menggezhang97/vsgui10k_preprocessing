...
class HybridPlainDecoderModel(nn.Module):
    def __init__(
        ...
        # lightweight crop encoder
        self.ui_crop_encoder = nn.Sequential(
        ...
        self.ui_crop_proj = nn.Linear(64, d_model)
    ...
    def encode_ui_crop(self, ui_crop_images):
        """
        ui_crop_images: [B, Ku, 3, Hc, Wc]
        return: [B, Ku, D]
        """
        B, Ku, C, H, W = ui_crop_images.shape
        x = ui_crop_images.view(B * Ku, C, H, W)
        x = self.ui_crop_encoder(x)          # [B*Ku, 64, 1, 1]
        x = x.flatten(1)                     # [B*Ku, 64]
        x = self.ui_crop_proj(x)             # [B*Ku, D]
        x = x.view(B, Ku, self.d_model)      # [B, Ku, D]
        return x

    def encode_ui_memory(self, ui_geom, ui_type_id, ui_mask, ui_crop_images, cue_token):
        """
        ui_geom:        [B, Ku, 4]
        ui_type_id:     [B, Ku]
        ...
        """
        geom_emb = self.ui_geom_encoder(ui_geom)              # [B, Ku, D]
        type_emb = self.ui_type_embedding(ui_type_id)         # [B, Ku, D]
        crop_emb = self.encode_ui_crop(ui_crop_images)        # [B, Ku, D]

        ui_tokens = torch.cat([geom_emb, type_emb, crop_emb], dim=-1)
    ...
    def forward(self, image, cue_id, history_xydur, ui_geom, ui_type_id, ui_mask, ui_crop_images):
        """
        image:         [B, 3, H, W]
        cue_id:        [B]
        history_xydur: [B, T, 3]
