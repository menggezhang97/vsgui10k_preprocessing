...
def load_ui_tokens(seg_json_path, ui_type_vocab, max_ui_tokens, drop_full_screen_root, seg_w=None, seg_h=None):
...
@torch.no_grad()
def predict_next(model, device, image_t, cue_id_t, ui_geom_t, ui_type_id_t, ui_mask_t, history_seq, history_len):
...
def teacher_forced_sequence(model, device, image_t, cue_id_t, ui_geom_t, ui_type_id_t, ui_mask_t, gt_fixations, history_len):
    ...
    for t in range(1, T):
        ...
        pred_xy = predict_next(
            model, device, image_t, cue_id_t,
            ui_geom_t, ui_type_id_t, ui_mask_t,
            hist, history_len
        )
...
def rollout_sequence(model, device, image_t, cue_id_t, ui_geom_t, ui_type_id_t, ui_mask_t, gt_fixations, history_len, rollout_steps=None):
    gt_seq = [[p[0], p[1]] for p in gt_fixations]
...
def draw_overlay(pil_img, gt_path, pred_path, out_path, title, target_bbox=None):
    ...
    plt.plot(gt_px[:, 0], gt_px[:, 1], "-o", color="deepskyblue", linewidth=2, markersize=4, label="GT")
    plt.plot(pred_px[:, 0], pred_px[:, 1], "-o", color="orangered", linewidth=2, markersize=4, label="Pred")
    ...
    if target_bbox is not None:
        ...
        tw = target_bbox["w"] * w
        th = target_bbox["h"] * h
        rect = plt.Rectangle((tx, ty), tw, th, fill=False, edgecolor="lime", linewidth=2, linestyle="--", label="Target")
...
def process_one_trial(
    ...
    ui_geom_t, ui_type_id_t, ui_mask_t = load_ui_tokens(
...
def main():
    ...
    parser.add_argument("--num_samples_per_split", type=int, default=10,
    ...
    with open(os.path.join(args.output_dir, "picked_trial_indices.json"), "w", encoding="utf-8") as f:
