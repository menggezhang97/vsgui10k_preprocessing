...
# UI mapping behavior
UI_FALLBACK_NEAREST = True
UI_NEAREST_THRESH_RATIO = 0.02  # 2% of seg-diagonal
...
def load_seg_elements(seg_path: str, W_seg: int, H_seg: int,
    ...
    for idx, c in enumerate(compos):
        ...
        # VSGUI10K uses (column_min/max, row_min/max)
    ...
    # Important: do NOT break early. Sort + truncate ensures we don't lose later boxes.
    out.sort(key=lambda z: (z["area"], z["idx"]))
    if len(out) > int(max_elements):
        out = out[:int(max_elements)]

    return out


def _point_to_bbox_dist(x, y, bbox):
    """
...
def map_fixation_to_ui(sx, sy, elements,
                       fallback_nearest=UI_FALLBACK_NEAREST,
                       nearest_thresh_ratio=UI_NEAREST_THRESH_RATIO):
    """
    Mapping rule:
      1) If inside one or more bboxes -> choose smallest area bbox (tie by idx).
      2) Else (optional) map to nearest bbox if distance <= thresh.
         thresh = nearest_thresh_ratio * diag(seg_image)

    Return:
      {"idx": int|None, "id": str|None, "class": str|None, "overlap": int,
       "dist": float|None, "mode": "inside"/"nearest"/"none"}
    """
    if not elements:
        return {"idx": None, "id": None, "class": None, "overlap": 0, "dist": None, "mode": "none"}
    ...
    # estimate diagonal from max extents of bboxes
    max_x, max_y = 0.0, 0.0
    for e in elements:
        x0, y0, x1, y1 = e["bbox"]
    ...
    thresh = float(nearest_thresh_ratio) * diag
    ...
    for e in elements:
        d = _point_to_bbox_dist(sx, sy, e["bbox"])
        if d < best_d - 1e-9:
            best = e
            best_d = d
            best_area = e["area"]
    ...
    return {"idx": None, "id": None, "class": None, "overlap": 0,
            "dist": float(best_d) if best is not None else None, "mode": "none"}
